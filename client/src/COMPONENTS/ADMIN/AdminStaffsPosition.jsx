...
class AdminStaffsPosition extends Component{
    constructor(){
        super();
        this.state = {
            staffs: [],
    ...
    postAndGetStaff = async () => {
        await axios.post('/admin-staff-position-post', {position: this.props.position})
            .then(() => this.getStaff());
    }
    ...
    getStaff = () => {
        ...
        fetch(`/admin-staff-position-get?token=${token}`)
            .then(res => res.json())
            .then(data => {
...
export default AdminStaffsPosition